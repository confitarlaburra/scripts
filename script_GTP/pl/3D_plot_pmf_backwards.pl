...
$ldelta = 0.1;
$lend   = 0.0; # lambda 1
$directory_name ="/storage/GTP/topo/TI/TI_long/backward"; # super directory fo LE freeze runs

open (OUT,">3D_plot.dat");
printf OUT "%10s   %5s   %10s\n", "#Angle","lambda","PMF";
while ($lstart >= $lend-1e-8) {
	$lstart = sprintf "%.1f",$lstart; 
        $file = "${directory_name}/LE_TI_LC_${lstart}/real_PMF/PMF_shifted.dat";
	$bool=0.0;
	open (FILE_1, "$file");
	while ($line=<FILE_1>) {
		if (($line=~/\s+(\S+)\s+\S+\s+(\S+)/) && ($bool ==1)) {
			$angle = $1;
			$pmf   = $2 + 0.38; # shift the pmf to zero (minimun value of all PMF curves)
			$lreal = (1-$lstart);
			$lreal = sprintf "%.1f",$lreal;
