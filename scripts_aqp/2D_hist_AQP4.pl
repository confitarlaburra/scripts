...
if ( ($ARGV[0] eq "") || ($ARGV[1] eq "" ) || ($ARGV[2] eq "" ) ||  ($ARGV[3] eq "" ) ||  ($ARGV[4] eq "" ))  
{
    ...
    print "Cases:\n";
    print "1 : Min distance SF(1D) and Dipole H201(2D)\n";
    print "2 : Distance CE(1D) and Dipole H95(2D)\n";
    print "3 : Min distance SF(1D) and Dihed H201(2D)\n"; 
    print "4 : Min distance CE(1D) and Dihed H95(2D)\n";
    print "Normalize:\n";
...
{
    # code
    @components=("0","SF_DIP201","CE_DIP095","SF_DIH201","CE_DIH095");

    $binNumber1D=$ARGV[2];
    $binNumber2D=$ARGV[3];
    $normalize=$ARGV[4];
    
# Min distance SF(1D) and Dipole H201(2D)
    if ($ARGV[1] eq "1") 
    {
	$D1b=45;  # MINSF
	$D2b=121; # DIPH201
    }
# Distance CE(1D) and Dipole H95(2D)
    if ($ARGV[1] eq "2") 
    {
	$D1b=55; #CE
	$D2b=143; #DIPH95
    }
# Min distance SF(1D) and Dihed H201(2D)
    if ($ARGV[1] eq "3") 
    {
	$D1b=45; 
	$D2b=64;
    }
# Min distance CE(1D) and Dihed H95(2D)
    if ($ARGV[1] eq "4") 
    {
	$D1b=55;
	$D2b=72;
...
Hist1D(\@values1D,$binNumber1D,substr($components[$ARGV[1]],0,2),$normalize);
Hist1D(\@values2D,$binNumber2D,substr($components[$ARGV[1]],3,6),$normalize);
### END MAIN ###
...
sub PrintGnuplot 
{
    ...
    $yMin=$_[3];
    $yMax=$_[4];
    $Xlabel=substr($name,0,2);
    $Ylabel=substr($name,3,6);	
